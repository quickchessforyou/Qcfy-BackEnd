...
export const createCompetition = async (req, res) => {
  try {
    const { name, description, startTime, endTime, duration, puzzles, maxParticipants } = req.body;
...
export const updateCompetition = async (req, res) => {
  try {
    ...
    if (updates.puzzles && updates.puzzles.length > 0) {
      const existingPuzzles = await PuzzleModel.find({ _id: { $in: updates.puzzles } });
...
export const submitSolution = async (req, res) => {
  try {
    ...
    if (!participant) {
      return res.status(400).json({ message: 'Not a participant in this competition' });
    }
    ...
    const puzzle = competition.puzzles.find(p => p._id.toString() === puzzleId);
    if (!puzzle) {
      return res.status(400).json({ message: 'Puzzle not part of this competition' });
    }
    
    // Validate solution (simplified - you can enhance this)
    const isCorrect = JSON.stringify(moves) === JSON.stringify(puzzle.solutionMoves);
